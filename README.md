...
So over one Christmas break, I built this: a zsh keymap system that puts those commands behind short, chainable aliases.
...
- **Namespaced aliases**: Each keymap claims one letter, e.g. `n` for Nav
...
`nn` lists the current directory, and every Nav listing is piped straight into Args, which numbers it. That numbering lets any command reach back and act on a listed entry, so `1 n` reads as "run `n` on arg #1":
...
`1 n` first echoes the command it resolved to, `n "CLAUDE.md"`, then runs it. `n` recognizes the `.md` explicitly and renders the file through `mdcat`; a `.csv` and `.json` extension would have rendered through Nav's own CSV parser and `jq` respectively.

The same `1 n` pattern works after any Args-backed listing, not just Nav's, e.g. `1 cat`, `1 rm`, or `1 open`.
...
This is the workflow from Example #1 laid bare: `nn` is `ls` piped into Args. Args then numbers every Nav listing for follow-up commands `<num> <command>`.
...
**Note:** `n dotfiles` only searches Nav's keymap entries when no file or directory named `dotfiles` exists in the CWD. Had one existed, `n <file>` and `n <directory>` would have taken precedence.
...
Run `ma` to list every keymap; run every keymap's bare alias to list its own usage and syntax legend, for example:
